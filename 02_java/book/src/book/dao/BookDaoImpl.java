...
/**
 * 도서 정보를 저장하는 Book 테이블에 여러 crud 작업을 
...
public class BookDaoImpl implements BookDaoIf {
	...
	public BookDaoImpl() {
		super();
		// getFactory() 메소드는 static 메소드 이므로 
	...
	public List<Book> select(Book book) {
		...
		// 지역변수로만 사용하는 것이 규칙
		SqlSession session = factory.openSession(); // insert = true
		
		try {
			// 1. 메퍼 인터페이스 세션으로부터 얻기
	...
	/**
	 * 입력된 책 일련번호에 해당하는 데이터가 존재하는 지 검사하여 
	 * 있으면 tru, 없으면 false 를 리턴하는 메소드
	 * @param bookSeq
	 * @return
	 */
